...
## Purpose
Own the bounded SKNR/SKNS protocol v1 shared by engine, neural worker, and the offline `neural-eval` evaluator.
...
./ci/run-test-quiet.ps1 -Name 'offline rerank adapter' -Command { cargo test -p sakura-conversion-eval --bin neural-eval --locked }
