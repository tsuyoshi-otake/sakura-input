...
#[test]
fn protocol_v21_hello_roundtrips_and_v20_payloads_are_rejected() {
    const PREVIOUS_PROTOCOL_VERSION: u16 = 20;
    assert_eq!(
        PROTOCOL_VERSION, 21,
        "the engine timing snapshot changes the request and response wire contract"
    );
    ...
    encode_request(&request, 20, &mut request_frame).expect("encode v21 request");
    assert_eq!(
    ...
    encode_response(&response, 20, &mut response_frame).expect("encode v21 response");
    assert_eq!(
