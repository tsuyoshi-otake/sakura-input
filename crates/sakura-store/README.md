...
- `learning::LearningLog`: synchronous durable learning transactions, including the existing two-`fs::rename` publication, recovery, exact operation ordering, and receipt-bearing terminal outcomes. Learning publication does not sync the parent directory and does not use `ReplaceFileW`.
...
- On-disk history framing constants (`HISTORY_MAGIC`, header and frame-header lengths) so settings and engine share the same layout numbers. The DPAPI-framed file scan itself stays in settings; adding a store history `read_snapshot` would exceed the 31-function budget.
- `crypto`: the `Sealer` boundary and current-user Windows DPAPI implementation.

## Must not own

- Asynchronous writer threads, queues, `InputHistoryService`, engine lifecycle, input-scope classification, CLI presentation, or the live history writer lock.
- Settings-owned history file scan, TSV presentation, or viewing retention. Those stay in `sakura-settings` so this crate does not grow a second snapshot reader.
...
Per-operation ordering remains the current ordering. `open` publishes a required format upgrade before runtime replay and truncates a recoverable tail after replay. Compaction and exact forget prepare their replacement runtime before publication. Exact forget publishes the replacement `LearningLog` and then invokes the engine publication callback before artifact cleanup; cleanup failure still returns `LogForget::Removed` with the receipt delta. `clear` publishes and reopens the empty canonical log, attempts the existing ignored-backup removal, and only then prepares the empty runtime. Structural extraction must not move these callbacks or add new replay error points.

Learning publication writes and `File::sync_all`s the temporary file, then performs the existing two `fs::rename` operations (canonical to backup/recovery, temporary to canonical). It does not sync the parent directory and does not claim `ReplaceFileW` or any stronger replace primitive. `ReplaceFileW` remains the history-persistence primitive only. `ReplayView::from_verified` consumes an already-verified prefix and does not introduce a second CRC failure terminal.
...
For learning, `learning/codec.rs` hosts the pre-extraction golden as a library unit test so default `cargo test -p sakura-store` still decodes the committed hex fixture and re-encodes its exact bytes. Engine learning regressions still cover upgrades, corruption, bounded reads, compaction, exact-forget fault recovery, clear and repair suppression. Settings owns TSV formatting and its exact escaping test. After the settings cutover, offline learning clear uses `LearningLog::clear` and settings has no normal `sakura-engine` edge. History file scan, TSV, and viewing retention remain settings-owned; the writer loop, queue, mutex, `Busy` admission, index, prediction history, sequence, generation and counters remain in engine.

The committed fixtures remain `crates/sakura-store/tests/fixtures/learning-v3-pre-extraction.hex` and `crates/sakura-store/tests/fixtures/learning-v3-pre-extraction.expected.tsv`. They preserve an actual synthetic `learning.bin` written locally by `bfa5eeb8a9a564f5d3401e7eec9321a2bca105d1` (file SHA-256 `7da8cb554c501bffe551e1f94a7501f1bb94e57cae9664a09d81494d3a0bc249`) and its exact TSV text (original LF SHA-256 `7fdd5700feebc5a08ff7ce0caa93ba065fcbe4e2225312fda01fce67956288b8`). That local writer revision is not reachable from the remote history; its `crates/sakura-engine/src/learning.rs` blob is `9abadc46f7885103c9a664811d2f2030dad8bef1`, identical to the remotely reachable PR #190 source revision `ca19035d4784f4ec2cd91421933acc8d8d4a4220`. Record `bfa5eeb8...` as the actual execution provenance and use the full `ca19035d...` revision as the reviewable source provenance. Decode the committed hex fixture, verify read and decode-to-encode identity, and compare exported TSV after newline normalization; do not regenerate the writer blob and do not require the unreachable commit during CI.
