...
fn oracle(facts: ShiftAsciiConvertFacts) -> ShiftAsciiConvertDecision {
    match (
        ...
        (false, _, _) => ShiftAsciiConvertDecision::Convert,
        (true, true, _) => ShiftAsciiConvertDecision::InsertLiteralSpace,
        (true, false, true) => ShiftAsciiConvertDecision::Convert,
        (true, false, false) => ShiftAsciiConvertDecision::RejectUnknown,
    }
}
...
fn concrete_decision_examples_match_the_independent_oracle() {
    let examples = [
        ...
        (
            "known English term still inserts a literal Space",
            ShiftAsciiConvertFacts {
                shifted_ascii: true,
                trigger_is_space: true,
                dictionary_hit: true,
...
fn shift_ascii_space_pbt_and_c2_cover_all_atomic_conditions() {
    ...
    let mut random = SEED;
    let mut condition_seen = [[false; 2]; 3];
    let mut combinations = [false; 8];
    ...
    for (index, combination) in combinations.iter_mut().enumerate() {
        let facts = ShiftAsciiConvertFacts {
            shifted_ascii: index & 1 != 0,
            trigger_is_space: index & 2 != 0,
            dictionary_hit: index & 4 != 0,
        };
        *combination = true;
        assert_eq!(decide_shift_ascii_convert(facts), oracle(facts));
    }
    ...
    eprintln!(
        "shift-ascii-space PBT seed={SEED:#018x}; random_cases=4096; exhaustive_cases=8; C2={covered}/{total}=100%"
    );
}
