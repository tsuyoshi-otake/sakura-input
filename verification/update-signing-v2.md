...
The signed object is the canonical `release-manifest-v2.txt`. Its detached
signature envelope is named `release-manifest-v2.sig` and is fetched from the
same trusted release asset set. A release is eligible only after manifest
parsing, envelope verification, downloaded-asset hash/size checks, and the
platform trust policy all succeed.
...
| `repository` | Exactly `tsuyoshi-otake/sakura-input`. |
| `channel` | Exactly `stable` for the production channel. |
| `platform` | Exactly `windows-x86_64` for this updater. |
...
Its checked-in public signature vector was generated by the offline signer
after the manifest bytes were frozen.
...
`standby` is a pre-pinned recovery key and may authorize a release within its
declared epoch and sequence window. Its role is an operational custody label,
not a network-controlled permission bit. `retired` is rejected for sequences
outside its closed window, and `revoked` is rejected for every sequence. An
unknown key ID is never upgraded into any trusted state. State changes are
shipped in a new updater/trust epoch; a release cannot change a key's state by
...
| --- | --- | --- | --- |
| `required` | Valid signature over the canonical manifest, accepted key/epoch/sequence | `WinVerifyTrust` returns success | Eligible, subject to all other guards |
| `required` | Missing, invalid, stale, unknown-key, or malformed | Any result | Reject |
| `unsigned` | Valid signature over the canonical manifest, accepted key/epoch/sequence | Exactly `TRUST_E_NOSIGNATURE` (`0x800b0100`) | Eligible, subject to all other guards |
...
| either | Any | Any other error, invalid/revoked signature, or policy mismatch | Reject |

The pinned Sakura manifest key supplies the application identity for both
rows. The `required` row additionally requires Windows' normal Authenticode
chain validation; a future certificate-specific Subject or SPKI pin may only
tighten that row. An unsigned row is an explicit owner-approved release policy,
not a general WinVerifyTrust bypass. `TRUST_E_SUBJECT_FORM_UNKNOWN`,
`TRUST_E_PROVIDER_UNKNOWN`, invalid, expired, or revoked signatures are not
treated as unsigned. A valid Authenticode signature cannot silently downgrade
or upgrade the signed policy.
...
canonical positive manifest, `manifest-positive.expected` records its digest,
and `signature-positive.txt` is a public P-1363 verification vector made by the
pinned active key. It contains no private key material. `manifest-tampered.txt`
changes only `size` and must fail the digest binding. `fixture-matrix.md`
defines malformed, reordered, envelope, key-binding, encoding, and
policy-downgrade cases. The implementation must run these cases before
accepting a release and must preserve the exact-file guard on every successful
path.
