...
param(
    ...
    [string[]]$Configs = @(
        'TsfProbeHostInsert-fix.cfg',
        'TsfProbeHostInsert-bug.cfg'
    )
)
...
$outputRoot = Join-Path $repoRoot 'verification\tsf-probe-host-insert\tlc'
...
foreach ($configName in $Configs) {
    ...
    $arguments = @(
        ...
        (Join-Path $modelDir 'TsfProbeHostInsert.tla')
    )
    ...
    [System.IO.File]::WriteAllText($stdout, $stdoutTask.Result)
    [System.IO.File]::WriteAllText($stderr, $stderrTask.Result)
    $expected = if ($slug -eq 'TsfProbeHostInsert-bug') { 12 } else { 0 }
