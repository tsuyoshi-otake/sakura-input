...
param(
    ...
    [string[]]$Configs = @(
        'DualTsfPhysicalKeyArbitration-bug-2state.cfg',
        'DualTsfPhysicalKeyArbitration-fix-3state.cfg',
        'DualTsfPhysicalKeyArbitration-fix-peer-first.cfg',
        'DualTsfPhysicalKeyArbitration-fix-owner-first.cfg',
        'DualTsfPhysicalKeyArbitration-fix-probe-failure.cfg',
        'DualTsfPhysicalKeyArbitration-fix-owner-teardown.cfg',
        'DualTsfPhysicalKeyArbitration-fix-context-replace.cfg',
        'DualTsfPhysicalKeyArbitration-reach-host-space.cfg'
    )
)
...
$outputRoot = Join-Path $repoRoot 'verification\dual-tsf-physical-key-arbitration\tlc'
[System.IO.Directory]::CreateDirectory($outputRoot) | Out-Null
$Configs = @($Configs | ForEach-Object { $_ -split ',' } | ForEach-Object { $_.Trim() } | Where-Object { $_ })

$expectCe = @(
    'DualTsfPhysicalKeyArbitration-bug-2state',
    'DualTsfPhysicalKeyArbitration-reach-host-space'
)
...
foreach ($configName in $Configs) {
    ...
    $arguments = @(
        ...
        (Join-Path $modelDir 'DualTsfPhysicalKeyArbitration.tla')
    )
    ...
    [System.IO.File]::WriteAllText($stdout, $stdoutTask.Result)
    [System.IO.File]::WriteAllText($stderr, $stderrTask.Result)
    $expected = if ($expectCe -contains $slug) { 12 } else { 0 }
