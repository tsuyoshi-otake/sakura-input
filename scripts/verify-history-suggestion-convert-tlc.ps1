...
param(
    ...
    [string[]]$Configs = @(
        'HistorySuggestionConvert-fix.cfg',
        'HistorySuggestionConvert-bug.cfg',
        'HistorySuggestionConvert-reach-identity.cfg'
    )
)
...
$outputRoot = Join-Path $repoRoot 'verification\history-suggestion-convert\tlc'
[System.IO.Directory]::CreateDirectory($outputRoot) | Out-Null
$Configs = @($Configs | ForEach-Object { $_ -split ',' } | ForEach-Object { $_.Trim() } | Where-Object { $_ })

$expectCe = @(
    'HistorySuggestionConvert-bug',
    'HistorySuggestionConvert-reach-identity'
)
...
foreach ($configName in $Configs) {
    ...
    $arguments = @(
        ...
        (Join-Path $modelDir 'HistorySuggestionConvert.tla')
    )
