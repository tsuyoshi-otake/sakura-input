...
ローマ字を入力し、`Space` で変換を開始します。続けて押すと次候補、`Shift`+`Space` で前候補へ移動します。既定では `変換` は漢字変換ではなく、§2.6 の文章変換です。`PageDown`／`PageUp` は候補ページ、`1`～`9` は表示ページ内の候補を直接選択します。`Enter` で確定、`Esc` または候補表示中の `Backspace` で変換前の読みへ戻ります。

ひらがなモードで最初の英字を`Shift`を押しながら入力すると、かなへ変えず英語用のcompositionを開始します。以降は`Shift`を離しても、そのcompositionを確定または取消するまで英語入力として扱います。英語用composition中の`Space`は常に半角の単語区切りを入力し、`変換`で辞書の正規表記を選びます。例: `Shift`+`C`の後に`laude`、`変換` → `Claude`／`Claude Code`、`Shift`+`O`の後に`penai`、`変換` → `OpenAI`、`Shift`+`P`の後に`ytorch`、`変換` → `PyTorch`。該当語がないときは変換せずビープし、`Enter`で入力した英字列をそのまま確定できます。この一時入力で、次の入力モードが英数へ変わることはありません。

文節は `←`／`→` で移動し、`Shift`+`←`／`Shift`+`→` で縮小・拡大します。

### 2.3 予測

入力中に候補が表示された場合、`Tab`／`Shift`+`Tab` または `↓`／`↑` で選び、`Enter` で確定します。先頭候補が入力中の表示と同じ場合、最初の`Tab`はそれを飛ばして最初の表示が変わる候補へ進みます。候補行を左クリックして直接確定することもできます。候補へフォーカスせず最上位を確定するには `Shift`+`Enter` を使います。端末向けアプリプロファイルでは、シェル補完を妨げないよう予測受理キーを無効にできます。

### 2.3.1 用語説明（Issue #28）

出典のある説明を持つ選択候補では、候補ポップアップの一定幅の詳細ペインに説明全文を折り返して表示します。候補一覧には長い説明を重複表示せず、`履歴`などの短い状態注記だけを残します。説明は右側、左側、下側の順に配置を試み、モニターの作業領域に物理的に収まらない場合だけ末尾を省略します。一覧と詳細は一つの不透明なウィンドウになるため、その外接矩形が入力中の文字列を覆う配置は使わず、詳細を出さない側を選びます。候補に説明がない場合や、複合候補など辞書の正確な項目へ解決できない場合も表示しません。
...
アプリで文字列を選択し、未入力状態で `変換` を押すと、既定では §2.6 の文章変換が始まります。文章変換キーを無効にしている場合、または対象が無い場合は、従来どおり再変換を要求します。アプリ側が TSF 再変換を実装していない場合は利用できません。確定直後の `Ctrl`+`Backspace` は確定を取り消します。取消可能期間を過ぎた場合はアプリへ通常のキーとして渡されます。

### 2.6 文章変換と校正

文章変換は明示操作だけで始まります。既定キーは JIS 配列の `変換` で、設定から `Caps Lock` か無効にも変更できます。入力中の文字列を優先し、それが無ければホストの空でない選択範囲を対象にします。対象が無いときは、そのキーの従来動作へ戻します。

校正はタスクバーの入力モード表示を右クリックし、「選択中文字列を校正」からだけ起動します。入力中は拒否します。

モデルは `gpt-5.6-luna` です。API キーは Windows Credential Manager だけに保存し、パスワード・URL・メール・数字・未分類の入力欄では起動しません。CLI や設定の失敗は、別モデルへフォールバックせずにエラーで終わります。
