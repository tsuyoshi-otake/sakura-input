...
ひらがなモードで`Shift`を押したまま英字を続けると、かなへ変えず英語用のcompositionを開始します。`Space`または`変換`で辞書の正規表記を選びます。例: `CLAUDE` → `Claude`／`Claude Code`、`OPENAI` → `OpenAI`、`PYTORCH` → `PyTorch`。該当語がないときは変換せずビープし、`Enter`で入力した大文字列をそのまま確定できます。この一時入力で、次の入力モードが英数へ変わることはありません。
