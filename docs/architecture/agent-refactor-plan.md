...
| **Allowed dependencies** | なし（`std` のみ） | `sakura-values`；target-specific `windows`（`persistence/` は `Win32_Foundation`／`Win32_Storage_FileSystem`、2.2c の `crypto/` は feature `dpapi`） | `sakura-values` | `sakura-engine`（dev）、`sakura-values`、`proptest` | `sakura-values`、`sakura-neural-proto` |
