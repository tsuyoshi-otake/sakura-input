...
| **Allowed consumers** | 全 crate | `sakura-engine`、`sakura-settings`、`tools/*` | `sakura-engine`、`sakura-neural-worker`、`dictc`（offline neural evaluation のみ） | `sakura-engine`（dev-dependency のみ）、cargo-mutants | `sakura-engine`（feature `context-research` のみ） |
