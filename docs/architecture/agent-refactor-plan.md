...
| 2.2d | learning codec／transaction 抽出後に settings を store に向ける | 1 change reason/PR を守り、(1) `LearningRecord`／`LearningSnapshot` と v1／v2／v3 の純 codec を store へ移し、先に `crates/sakura-store/tests/fixtures/learning-v3-pre-extraction.hex` と同 `.expected.tsv` を固定、(2) atomic publication／recovery と現行 callback 順序を保つ同期 `LearningLog` を移し、transaction の success と error の双方で exact maintenance-failure delta を持つ receipt を返す、(3) settings の learning／input_history 永続データ参照を store に向けて `Cargo.toml` から `sakura-engine` を外す、の順で実施する。engine は runtime index／prediction history／repair suppress、sequence allocation、`Busy`、counter／mutex／maintenance thread、scoring／writer lifecycle を保持する。offline learning clear は `LearningLog::clear` を使い、TSV formatting は settings に置く | (1) fixture の decode→encode identity と `.expected.tsv` byte 一致、store codec tests、(2) failure injection を含む store transaction tests と既存 engine learning tests、(3) R2；`cargo tree -p sakura-settings -e normal \| rg engine`；`cargo test -p sakura-settings`；learning view/export/offline clear と `history show/export/stats/clear` の統合テスト | v1／v2／v3 bytes と TSV 不変。各 transaction の既存 publication／replay／cleanup 順序不変。全 transaction success/error が receipt を持ち、`FilteredCanonical` は cleanup error 後も committed／`Removed`、旧 runtime を保持する recovery error は error のまま。R2 空、settings tree に engine なし、offline clear と TSV pass、`learning.bin`／`input.bin` 形式不変 | L（順序付き 3 PR） |
