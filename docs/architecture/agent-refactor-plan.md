...
- **休眠コード**：engine の #34 系 4 モジュール（`context_baseline/evaluation/intelligence`＋α、1,846 行）は常に compile され、oracle は 3 本と self-test 3 本。うち space-key oracle が常時 compile されていた（Phase 2.4 で dev 専用 crate へ移動）。
...
```
 ...
 dev only        sakura-oracles (engine の TLA+ oracle 3 本＋self-test 3 本)   dictc-core / dictc(bin)   tools/*
```
...
| `sakura-store` | learning／input_history の **format（record layout・上限）／codec／persistence（ファイル配置・保持・compaction の純関数）／crypto（DPAPI を trait の背後に）**と、learning の**同期 durable transaction**。writer thread・queue・runtime index／scoring は engine に残す | engine §7、renderer-settings §5、contracts §6 の `sakura-stores` を統合。名前は `sakura-store` に統一 | 必須 |
| `sakura-rerank-proto` | `SKNR`/`SKNS` v1（magic、frame、bounds） | contracts §6 | 必須 |
| `sakura-oracles` | engine の `*_oracle.rs` 3 本＋self-test 3 本（dev-dependency のみ） | engine §7 | 推奨 |
...
| **Owns** | `FixedStr`／`FixedVec`／`Overflow`、上限定数（`MAX_CANDIDATES`、`MAX_PREEDIT_BYTES`、`MAX_SEGMENTS` ほか）、`Mode`／`KeyCode`／`KeyInput`／`Modifiers`、`AppearanceTheme`／`PadShortcut`、`InputScope`、`AiTextOperation`／`AiTextStatus`、`Fingerprint` | `input_history::format`（record layout、version、tag 0／1／2 の `HistoryScope`、`MAX_RECORD_BYTES`）、`input_history::codec`（record の encode・decode）、`persistence`（`input.bin` 配置、atomic write、30 日保持、64 MiB 上限、compaction の純関数）、`crypto`（`Sealer` trait と `DpapiSealer`）、`learning` の純 codec と同期 durable transaction（既存の二段 `fs::rename` publication／recovery／現行 operation ordering／success・error の receipt。learning は親ディレクトリ sync も `ReplaceFileW` も持たない） | magic、frame header、bounds、request／response の encode・decode、version 定数 | `*_oracle.rs` 3 本＋self-test 3 本と oracle 用 fixture | 現 engine の休眠 4 モジュール 1,846 行のみ |
...
| **Allowed dependencies** | なし（`std` のみ） | `sakura-values`；target-specific `windows`（`persistence/` は `Win32_Foundation`／`Win32_Storage_FileSystem`、2.2c の `crypto/` は feature `dpapi`） | `sakura-values` | `sakura-engine`（dev）、`sakura-values`、`proptest` | `sakura-values`、`sakura-context-proto` |
| **Allowed consumers** | 全 crate | `sakura-engine`、`sakura-settings`、`tools/*` | `sakura-engine`、`sakura-neural-worker`、offline `neural-eval`（現 `dictc`、Phase 3.9 後は `tools/` の移動先 package） | `sakura-engine`（dev-dependency のみ）、cargo-mutants | `sakura-engine`（feature `context-research` のみ） |
| **Public API budget** | `pub` 型 ≤25、`pub fn` は値の構築・変換のみ（I/O 0） | 最終 `pub` 型 ≤20、`pub fn` ≤31。learning の純 codec 段階は型 14／関数 31（既存 11／23 に `LearningRecord`／`LearningSnapshot`／一時的 `DecodedRecord` と低水準 codec 8 関数）。transaction 移動時に `DecodedRecord` を `LearningLog`／`ReplayView`／`ReplayEvent`／`OperationReceipt`／`LearningLogError`／`LogMaintenance`／`LogForget` へ置換し、低水準 8 関数を crate-private にして同数の `memory`／`append`／`append_repair_suppress`／`read_snapshot`／`open`／`maintain`／`forget_exact`／`clear` を公開する。2.2b の 6 入口と 2.2c の `Sealer::seal`／`open` を含む | `pub` item ≤8 | `pub mod` 12、`pub fn` は oracle 入口のみ | `pub` item ≤6 |
...
| 2.2b | `sakura-store::input_history::persistence`（保持・compaction の純関数とファイル配置） | `RETENTION` 30 日、`MAX_INPUT_HISTORY_BYTES` 64 MiB、compaction の age filter／sequence ordering を `persistence.rs` の **入力 `Vec<InputHistoryRecord>` → 出力 `Vec<InputHistoryRecord>`** ownership-consuming 純関数へ（現行 compaction の所有移動を保ち、durable content を clone しない）。64 MiB 選別は実 DPAPI frame 長を使うため、2.2c までは engine が protected frame を準備し、store の上限定数／overflow 判定を使う。`%LOCALAPPDATA%\SakuraInput\history\input.bin` の path 規則、compaction transaction path、Windows の exact `ReplaceFileW` primitive も同モジュール。writer loop・queue・`InputHistoryService`・`ScopeClass`、未解決 marker admission、DPAPI、frame scan、publication checkpoint は **engine に残す**（actor は 4.1、crypto は 2.2c） | `cargo test -p sakura-store --lib persistence`；`cargo test -p sakura-store --no-default-features`；既存 engine input-history tests；TLA+ `DeveloperHistory` の retention/actor correspondence | pure retention／size／path tests は非 Windows でも compile/pass。Windows で replacement primitive pass。固定 payload byte、privacy、publication failure、writer terminal semantics 不変 | M |
...
| 2.2d | learning codec／transaction 抽出後に settings を store に向ける | 1 change reason/PR を守り、(1) `LearningRecord`／`LearningSnapshot` と v1／v2／v3 の純 codec を store へ移し、先に `crates/sakura-store/tests/fixtures/learning-v3-pre-extraction.hex` と同 `.expected.tsv` を固定、(2) 既存の二段 `fs::rename` publication／recovery と現行 callback 順序を保つ同期 `LearningLog` を移し、transaction の success と error の双方で exact maintenance-failure delta を持つ receipt を返す。learning 側に親ディレクトリ sync や `ReplaceFileW` を追加しない、(3) settings の learning／input_history 永続データ参照を store に向けて `Cargo.toml` から `sakura-engine` を外す、の順で実施する。engine は runtime index／prediction history／repair suppress、sequence allocation、`Busy`、counter／mutex／maintenance thread、scoring／writer lifecycle を保持する。offline learning clear は `LearningLog::clear` を使い、TSV formatting は settings に置く | (1) fixture の decode→encode identity と `.expected.tsv` の改行正規化後一致、store codec tests、(2) failure injection を含む store／engine learning tests、(3) R2；`cargo tree -p sakura-settings -e normal \| rg engine`；`cargo test -p sakura-settings`；learning view/export/offline clear と `history show/export/stats/clear` の統合テスト | v1／v2／v3 bytes と TSV 不変。各 transaction の既存 publication／replay／cleanup 順序不変（二段 rename、directory sync なし）。全 transaction success/error が receipt を持ち、`FilteredCanonical` は cleanup error 後も committed／`Removed`、旧 runtime を保持する recovery error は error のまま。R2 空、settings tree に engine なし、offline clear と TSV pass、`learning.bin`／`input.bin` 形式不変 | L（順序付き 3 PR） |
| 2.3 | `sakura-rerank-proto` | `neural-worker/src/protocol.rs`、`engine/src/long_conversion.rs`、`dictc/src/bin/neural_eval.rs` の codec を 1 crate に（`Fingerprint` は 2.1c 済み）。README・R8 更新・`benchmarks.json` 同梱 | R8；quiet wrapper 経由で `cargo test -p sakura-neural-worker -p sakura-rerank-proto`、engine 対象テスト、`cargo test -p dictc --bin neural-eval`；2 候補 protocol v1 IPC テスト | protocol owner は 1 件。worker、engine、offline neural-eval が同じ定義を使い既存テストが通る | S |
| 2.4 | `sakura-oracles` | oracle 3 本と self-test 3 本を dev-dependency crate へ。常時 compile を削除。cargo-mutants 設定・generator・test consumer を repoint。README 同梱。R9 は専用 crate 以外の oracle と非 dev 依存を拒否 | `cargo build -p sakura-engine --release` 後に今回生成の engine `.d` と normal dependency tree を検査；`cargo test -p sakura-engine -p sakura-oracles`；R9 self-test／enforce；既存の限定 mutation campaign | release の engine dependency に oracle 無し（symbol scan とは区別）。engine＋oracle 合計の test identity／件数不変。R9 pass | S |
| 2.5 | `sakura-context-research` | #34 休眠 4 モジュール 1,846 行を feature `context-research` 付き別 crate へ。README の Owns は 4 モジュール限定、Must not own に「既定 engine build から参照されるもの」。既存 budget test も crate 内へ移し、engine の Session サイズ診断と候補表示上限の検証を feature 付きテストに残す。1 年間 feature が使われなかったことを確認した場合だけ、所在を `docs/history/research/` に記録する別の削除 PR で廃止する | default／feature 付き engine build；quiet wrapper で research crate と feature 付き engine test；default `cargo tree -p sakura-engine -e normal` と fresh release depfile；公開 API／旧参照検索 | default engine graph／depfile に research がない。feature 付きで研究コードが compile でき、19 個の元 unit test と 2 個の allocation test が PASS、release-only benchmark 1 個の ignore を維持。外部公開 API は 0。候補表示上限 9 の実 engine assertion を保持。workspace 全体の検証では研究 crate も対象とする | S（D13 決定済み、2026-09-13 owner 委任） |
| 2.6 | engine dev-deps 整理 | `ime-eval` dev-dep を削除し、`real_engine_candidate_capture_round_trip` を ime-eval の integration test へ移動。package-local `ime_eval_sakura_engine` は実 engine entry point を `engine-fixture` 時だけビルド。`dictc` は engine の `dev-fixtures` 背後とし、既存 7 unit module／8 integration target の選択を CI・release・基準値 recorder の明示 feature で維持 | `cargo tree -p sakura-engine -e dev`；`cargo tree -p sakura-engine -e normal`；quiet wrapper で `cargo test -p sakura-engine --features dev-fixtures` と `cargo test -p sakura-ime-eval --features engine-fixture`；`ci/record-test-baseline.ps1 -SelfTest` と実採取 | engine dev graph に ime-eval が出ず、通常 graph に dictc が出ない。移動した test は単一 owner。既存 fixture test が省略されず pass。基準 JSON は default と明示 feature を区別して実行結果を記録 | S |
...
| 2.8 | R1・R2・R8・R12・R13 を blocking に | 0.3 の `-Advisory` から外し、CI は `-Enforce R1,R2,R8,R9,R12,R13`。R3〜R7 は引き続き advisory | `pwsh ./ci/check-dependency-rules.ps1 -SelfTest`；`pwsh ./ci/check-dependency-rules.ps1 -Advisory -Enforce R1,R2,R8,R9,R12,R13` | 違反 0。SelfTest の negative fixture と CI Enforce 行が残る | S |
...
| `sakura-context-proto` | `sakura-neural-proto`（旧名） |
