...
13. **Four dormant #34 modules (1,846 lines / 1,212 prod) sit in the engine's compile+lint unit but are unreachable from `dispatch`/`server`.** Boundary: move to a `sakura-context` crate.
...
- **`crates/sakura-store`** — record types, codec, DPAPI, snapshot readers, format constants for both stores. Depends on `sakura-proto` + `windows` only. `sakura-settings` depends on this instead of `sakura-engine`. Binds `verification/history-*.md`.
- **`crates/sakura-context`** — the four dormant #34 modules. Depends on `sakura-neural-proto`, `sakura-proto`.
...
Dependency direction (top depends on bottom, one-way): `ipc/` → `request/` → `keys/`,`commit/`,`render/` → `conversion/`,`candidates/`,`predict/` → `edit/`,`state/` → `services/`,`runtime/` → `sakura-core`/`sakura-store`/`sakura-proto`. `state/` must not import anything above it (enforceable: `rg -n 'use crate::(keys|commit|render|ipc|request)' crates/sakura-engine/src/state/` returns nothing). Convention: unit tests as `src/<area>/<module>_tests.rs` via `#[cfg(test)] #[path=…] mod tests;`; cross-module and real-process tests as `tests/<area>_<topic>.rs`.
...
2. **Same for `learning.rs`, `input_history.rs`, `server.rs`, `session.rs`, `prediction.rs`, `ui.rs`, `dictionary.rs`.** Risk: low. `Verify:` `cargo test -p sakura-engine --lib` `Expect:` same test count; no production `src/*.rs` over 2,400 lines.
3. **Extract `crates/sakura-store`** with `input_history` ranges 36–654, 1143–1214, 1543–1616, 1778–1975 and `learning`'s snapshot reader; re-export from `sakura_engine::{input_history,learning}` so callers are unchanged, then repoint `sakura-settings`. Risk: medium (DPAPI + on-disk format). **Owner decision:** whether settings drops the engine dep in the same PR. `Verify:` `rg -n 'sakura_engine::' crates/sakura-settings` `Expect:` no matches; `cargo test -p sakura-store -p sakura-settings` passes.
...
5. **Move the 4 dormant #34 modules into `crates/sakura-context`.** Risk: low. `Verify:` `rg -n 'context_(baseline|evaluation|intelligence)|prediction_snapshot' crates/sakura-engine/src` `Expect:` nothing outside `lib.rs` re-exports.
...
13. **Split `Session` (`session.rs:262`) into `state/{composition,conversion_state,undo,scope}.rs` sub-structs.** Risk: high (touches every dispatch call site); **owner decision required.** `Verify:` `cargo test -p sakura-engine` + `--test zero_alloc_dispatch` `Expect:` no new allocations, all tests pass.
...
15. **Drop the `sakura-ime-eval` dev-dependency and gate `dictc` behind a `dev-fixtures` feature.** Risk: low. `Verify:` `cargo test -p sakura-engine --no-run` `Expect:` builds; `rg -n 'ime_eval' crates/sakura-engine` returns nothing.
